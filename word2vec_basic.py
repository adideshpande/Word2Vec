...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  """
    ...
    while (data_index + window_size < len(data)):
        # put data in the batch such that you start with context word at data_index and then,
        # pick a word from one-index left and one-index right
        for x in range(1, num_skips // 2 + 1):
            batch[y], labels[y] = data[skip_window + data_index], data[(skip_window - x) + data_index]
            y = y + 1
            batch[y], labels[y] = data[skip_window + data_index], data[(skip_window + x) + data_index]
            y = y + 1
            if y == batch_size:
